...
import { Baby, Users, Phone, HeartPulse, Sparkles, CheckCircle2, ArrowLeft, ArrowRight } from "lucide-react";
...
export const Route = createFileRoute("/enroll")({
  ...
  errorComponent: ({ error }) => (
    <div role="alert" className="mx-auto max-w-md p-10 text-center font-medium text-muted-foreground">
      {error.message}
    </div>
  ),
...
function EnrollPage() {
  ...
  const toggleService = (name: string, on: boolean) => {
    setSelectedServices((prev) => (on ? [...new Set([...prev, name])] : prev.filter((s) => s !== name)));
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 0) {
      ...
      else if (!PHONE_RE.test(f.parent_phone.trim())) e.parent_phone = "Enter a valid phone number.";
      if (!f.parent_email.trim()) e.parent_email = "Email is required.";
      else if (!EMAIL_RE.test(f.parent_email.trim())) e.parent_email = "Enter a valid email address.";
      ...
      if (f.ec2_phone.trim() && !PHONE_RE.test(f.ec2_phone.trim())) e.ec2_phone = "Enter a valid phone number.";
    } else if (s === 3) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-2xl px-5 py-16 lg:py-24">
        {done ? (
          <ConfirmationCard childName={f.child_full_name.trim()} />
        ) : (
          <>
...
function StepChild({ f, set, errors }: StepProps) {
  ...
  return (
    <StepShell icon={<Baby className="h-5 w-5" />} tone="primary" title="Child's Information" subtitle="Tell us about your little one">
      <Field id="child_full_name" label="Full name" required error={errors.child_full_name}>
        <Input id="child_full_name" value={f.child_full_name} onChange={(e) => set("child_full_name", e.target.value)} placeholder="e.g. Amara Wanjiru" />
      </Field>
      <div className="grid gap-5 sm:grid-cols-2">
        <Field id="child_dob" label="Date of birth" required error={errors.child_dob} hint={age || undefined}>
          <Input id="child_dob" type="date" value={f.child_dob} onChange={(e) => set("child_dob", e.target.value)} />
        </Field>
        <Field id="child_gender" label="Gender" optional>
          <Select value={f.child_gender} onValueChange={(v) => set("child_gender", v)}>
            <SelectTrigger id="child_gender"><SelectValue placeholder="Select…" /></SelectTrigger>
            <SelectContent>
      ...
      <Field id="child_nickname" label="Preferred name / nickname" optional>
        <Input id="child_nickname" value={f.child_nickname} onChange={(e) => set("child_nickname", e.target.value)} placeholder="e.g. Ama" />
      </Field>
    </StepShell>
  );
}
...
function StepGuardian({ f, set, errors }: StepProps) {
  return (
    <StepShell icon={<Users className="h-5 w-5" />} tone="secondary" title="Parent / Guardian" subtitle="Primary contact information">
      <Field id="parent_full_name" label="Full name" required error={errors.parent_full_name}>
        <Input id="parent_full_name" value={f.parent_full_name} onChange={(e) => set("parent_full_name", e.target.value)} placeholder="e.g. Grace Kamau" />
      </Field>
      <div className="grid gap-5 sm:grid-cols-2">
        <Field id="parent_relationship" label="Relationship to child" required error={errors.parent_relationship}>
          <Select value={f.parent_relationship} onValueChange={(v) => set("parent_relationship", v)}>
            <SelectTrigger id="parent_relationship"><SelectValue placeholder="Select…" /></SelectTrigger>
            <SelectContent>
        ...
        <Field id="parent_phone" label="Phone number" required error={errors.parent_phone}>
          <Input id="parent_phone" type="tel" value={f.parent_phone} onChange={(e) => set("parent_phone", e.target.value)} placeholder="+254 7XX XXX XXX" />
        </Field>
      </div>
      <Field id="parent_email" label="Email address" required error={errors.parent_email}>
        <Input id="parent_email" type="email" value={f.parent_email} onChange={(e) => set("parent_email", e.target.value)} placeholder="grace@email.com" />
      </Field>
      <Field id="home_address" label="Home address" required error={errors.home_address}>
        <Textarea id="home_address" value={f.home_address} onChange={(e) => set("home_address", e.target.value)} placeholder="Estate, street, city" rows={3} />
      </Field>
    </StepShell>
  );
}
...
function StepEmergency({ f, set, errors }: StepProps) {
  return (
    <StepShell icon={<Phone className="h-5 w-5" />} tone="destructive" title="Emergency Contacts" subtitle="People we can reach if we can't reach you">
      <div className="rounded-2xl border-2 border-border p-5">
        <p className="mb-4 font-display text-base font-extrabold text-foreground">Emergency Contact 1</p>
        <div className="grid gap-5">
          <div className="grid gap-5 sm:grid-cols-2">
            <Field id="ec1_name" label="Full name" required error={errors.ec1_name}>
              <Input id="ec1_name" value={f.ec1_name} onChange={(e) => set("ec1_name", e.target.value)} placeholder="e.g. John Mwangi" />
            </Field>
            <Field id="ec1_relationship" label="Relationship" required error={errors.ec1_relationship}>
              <Input id="ec1_relationship" value={f.ec1_relationship} onChange={(e) => set("ec1_relationship", e.target.value)} placeholder="e.g. Uncle" />
            </Field>
          </div>
          <Field id="ec1_phone" label="Phone number" required error={errors.ec1_phone}>
            <Input id="ec1_phone" type="tel" value={f.ec1_phone} onChange={(e) => set("ec1_phone", e.target.value)} placeholder="+254 7XX XXX XXX" />
          </Field>
        </div>
      </div>
      <div className="rounded-2xl border-2 border-border p-5">
        <p className="mb-4 font-display text-base font-extrabold text-foreground">
          Emergency Contact 2 <span className="text-sm font-medium text-muted-foreground">(optional)</span>
        </p>
        <div className="grid gap-5">
          <div className="grid gap-5 sm:grid-cols-2">
            <Field id="ec2_name" label="Full name" optional>
              <Input id="ec2_name" value={f.ec2_name} onChange={(e) => set("ec2_name", e.target.value)} placeholder="e.g. Mary Njeri" />
            </Field>
            <Field id="ec2_relationship" label="Relationship" optional>
              <Input id="ec2_relationship" value={f.ec2_relationship} onChange={(e) => set("ec2_relationship", e.target.value)} placeholder="e.g. Aunt" />
            </Field>
          </div>
          <Field id="ec2_phone" label="Phone number" optional error={errors.ec2_phone}>
            <Input id="ec2_phone" type="tel" value={f.ec2_phone} onChange={(e) => set("ec2_phone", e.target.value)} placeholder="+254 7XX XXX XXX" />
          </Field>
        </div>
      </div>
    </StepShell>
...
function StepMedical({ f, set, errors }: StepProps) {
  return (
    <StepShell icon={<HeartPulse className="h-5 w-5" />} tone="sky" title="Medical Info & Allergies" subtitle="Help us keep your child safe">
      <Field id="allergies" label="Known allergies" required error={errors.allergies} hint='Enter "None" if not applicable'>
        <Textarea id="allergies" value={f.allergies} onChange={(e) => set("allergies", e.target.value)} placeholder="e.g. Peanuts, latex — or write 'None'" rows={2} />
      </Field>
      <Field id="medications" label="Current medications" optional>
        <Textarea id="medications" value={f.medications} onChange={(e) => set("medications", e.target.value)} placeholder="Any medications your child is currently taking…" rows={2} />
      </Field>
      <Field id="medical_conditions" label="Medical conditions" optional>
        <Textarea id="medical_conditions" value={f.medical_conditions} onChange={(e) => set("medical_conditions", e.target.value)} placeholder="Any conditions or special needs our team should know about…" rows={2} />
      </Field>
      <div className="grid gap-5 sm:grid-cols-2">
        <Field id="doctor_name" label="Doctor's name" optional>
          <Input id="doctor_name" value={f.doctor_name} onChange={(e) => set("doctor_name", e.target.value)} placeholder="Dr. Otieno" />
        </Field>
        <Field id="doctor_phone" label="Doctor's phone number" optional>
          <Input id="doctor_phone" type="tel" value={f.doctor_phone} onChange={(e) => set("doctor_phone", e.target.value)} placeholder="+254 7XX XXX XXX" />
        </Field>
      </div>
    </StepShell>
  );
...
function StepServices({
  ...
  return (
    <StepShell icon={<Sparkles className="h-5 w-5" />} tone="primary" title="Service Preference & Consent" subtitle="Select all services your child will use">
      <div>
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          {services.map((s) => {
            ...
            return (
              ...
              >
                <Checkbox checked={checked} onCheckedChange={(v) => toggle(s.name, v === true)} className="mt-1" />
                <div>
                  <p className="font-display text-base font-extrabold text-foreground">{s.name}</p>
                  {s.description && <p className="mt-1 text-sm font-medium text-muted-foreground">{s.description}</p>}
                </div>
              </label>
            );
          })}
        </div>
        {errors.services && <p className="mt-2 text-sm font-semibold text-destructive">{errors.services}</p>}
      </div>

      <div className="grid gap-5 sm:grid-cols-2">
        <Field id="preferred_start_date" label="Preferred start date" required error={errors.preferred_start_date}>
          <Input id="preferred_start_date" type="date" value={f.preferred_start_date} onChange={(e) => set("preferred_start_date", e.target.value)} />
        </Field>
        <Field id="dropoff_time" label="Typical drop-off time" optional>
          <Input id="dropoff_time" type="time" value={f.dropoff_time} onChange={(e) => set("dropoff_time", e.target.value)} />
        </Field>
      </div>
      ...
      {errors.consent && <p className="-mt-2 text-sm font-semibold text-destructive">{errors.consent}</p>}
    </StepShell>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center gap-4">
        <span className={cn("grid h-11 w-11 place-items-center rounded-2xl", toneMap[tone])}>{icon}</span>
        <div>
...
function ConfirmationCard({ childName }: { childName: string }) {
  return (
    <div className="mx-auto max-w-xl text-center">
      <div className="rounded-3xl bg-card p-10 shadow-card">
        ...
        <h1 className="mt-6 font-display text-2xl font-extrabold text-foreground sm:text-3xl">
          Thank you — we've received {childName ? `${childName}'s` : "your child's"} registration
        </h1>
        <p className="mt-4 font-medium text-muted-foreground">
          Our team will be in touch shortly to confirm the next steps. Welcome to the KIDS' NOOK family!
        </p>
